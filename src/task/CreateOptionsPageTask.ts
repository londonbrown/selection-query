...
export default class CreateOptionsPageTask {
    async run() {
        ...
        const categoriesPlaceholderText = JSON.stringify(new MenuCategory(
            categoryId,
            MenuCategoryType.CATEGORY,
            "Example Parent Menu",
        ), undefined, 2)
        const categoriesJsonEditor = this.createJsonEditor("menuCategories",
        ...
        const itemsJsonEditor = this.createJsonEditor("menuItems",
    ...
    createJsonEditor(key: string, placeholder: string, value?: string): HTMLTextAreaElement {
    ...
    createButtonGroup(key: string): HTMLDivElement {
        ...
        const saveButton = this.createButton(`save-${key}`, "Save", () => this.onSave(key))
        const importButton = this.createButton(`import-${key}`, "Import", () => this.onImport(key))
        const exportButton = this.createButton(`export-${key}`, "Export", () => this.onExport(key))
    ...
    createButton(id: string,
                          text: string,
                          onclick: (this: GlobalEventHandlers, ev: MouseEvent) => any): HTMLButtonElement {
    ...
    onSave(key: string) {
        ...
        let editor = document.getElementById(`${key}-editor`) as HTMLTextAreaElement
        let editorText = editor.value.trim()
        if (editorText) {
            browser.storage.local.set({[key]: JSON.parse(editorText)})
                .then(() => {
                    console.log("OK")
                    new CreateMenuTask().run().then(r => {
