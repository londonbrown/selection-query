...
export default class CreateMenuTask {
    ...
    async run() {
        ...
        console.log(`Building menu from ${menuCategories.length} categories and ${menuItems.length} items`)
        for (let menuCategory of await MenuItemsDAO.getInstance().getCategories()) {
        ...
        for (let menuItem of await MenuItemsDAO.getInstance().getItems()) {
    ...
    private static onItemCreated(menuComponent: MenuItem|MenuCategory): string | number {
        console.log(`Created menu component of type ${menuComponent.type}: ${menuComponent.id}`)
    ...
    private static onItemClick(info: Menus.OnClickData) {
        ...
        let menuItem = MenuItemsDAO.getInstance().getItems().then(value => value.find(menuItem => {
