...
export default class MenuItemsDAO {
  ...
  async getCategories(): Promise<Array<MenuCategory>> {
    return this.getMenuComponents(
      "menuCategories",
      MenuCategory.fromObjectProperties
    )
  }
  ...
  private async getMenuComponents(
    key: "menuCategories" | "menuItems",
    serializer: (menuComponentProps: never) => MenuItem | MenuCategory
  ) {
    const menuComponentItem = await browser.storage.local.get(key)
    if (menuComponentItem[key]) {
      return menuComponentItem[key].map((menuComponentProps: never) => {
